...
If widespread dead code appears beyond the user's requested area, switch to full-repo `Audit` mode before deleting more. If the repo also uses continuity docs such as `AGENTS.md`, add concise notes about flagged but unremoved dead code so future agents do not rediscover the same uncertain candidates from scratch.
